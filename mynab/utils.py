...
def get_excluded_groups():
    ...
    return ['Internal Master Category', 'Uncategorized', 'Credit Card Payments', 'Hidden Categories']
...
def create_unified_plot(name, transactions_df, budget_df, global_month_range, categories_data, plot_type='category'):
    """
    ...
    if plot_type == 'category_group':
        filtered_transactions = transactions_df[transactions_df['category_group'] == name].copy()
        ...
        filtered_transactions = transactions_df[transactions_df['category'] == name].copy()
