...
def process_categories_data(categories_response):
    """Process categories data into a structured format"""
    categories_data = []
    category_groups = {}
...
def create_category_group_summary(transactions_df, budget_df):
    ...
    # Filter out transactions with empty category groups and income transactions
    filtered_df = transactions_df[(transactions_df['category_group'] != '') & (~transactions_df['is_income'])].copy()
...
def create_group_trends_plot(transactions_df, plot_type='moving_average'):
    ...
    # Filter out transactions with empty category groups for expenses
    expense_df = transactions_df[(transactions_df['category_group'] != '') & (~transactions_df['is_income'])].copy()
...
def main():
    ...
    group_summary_fig = create_category_group_summary(transactions_df, budget_df)
    ...
    with col1:
        ...
        moving_avg_fig = create_group_trends_plot(transactions_df, 'moving_average')
        st.plotly_chart(moving_avg_fig, use_container_width=True)
    
    with col2:
        st.subheader("🔮 Forecast Trends by Group")
        forecast_fig = create_group_trends_plot(transactions_df, 'forecast_trend')
        st.plotly_chart(forecast_fig, use_container_width=True)
    
    st.markdown("---")
    
    # Individual Category Analysis
    st.header("📋 Individual Category Analysis")
    ...
    selected_category = st.selectbox(
        ...
        category_names,
        index=0 if category_names else None
    )
    
    if selected_category:
        category_fig = create_category_plot(selected_category, transactions_df, budget_df)
        if category_fig:
